...
<body>
	<?php
		...
		$db = new mysqli('localhost', 'flick', 'Flick1dothome!') or die('Unable connect to database.');

		if ($db)
		{
			$db->query("SET NAMES utf8");
			mysqli_select_db($db, 'flick');
			$db->query($query);
		}
		mysqli_close($db);
		...
		echo("<script>location.href='http://www.flicklab.co.kr/';</script>");
	?>
</body>
</html>